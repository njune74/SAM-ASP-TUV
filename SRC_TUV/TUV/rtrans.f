...
*  specific to psndo:
      ...
      PARAMETER(MAXCLY=201,MAXULV=201)
...
c     .. Parameters ..
      ...
      PARAMETER ( MXCLY = 201, MXULV = 201, MXCMU = 32, MXUMU = 32,
...
* delat-M scaling option
      ...
      write(*,*) "PSNDO Check 1", PASS1

      IF( PASS1 ) THEN
         write(*,*) "PSNDO Check 1.3", PASS1, epsilon(4.)
         call flush(6)
         !DITHER = 10.*R1MACH( 4 ) !MJA this function is hangling for some reason, replace with F90 version

         DITHER = epsilon(4.)
         write(*,*) "PSNDO Check 1.5", DITHER
         call flush(6)
c                            ** Must dither more on Cray (14-digit prec)

         IF( DITHER.LT.1.E-10 ) DITHER = 10.*DITHER
         write(*,*) "PSNDO Check 1.7", DITHER
         call flush(6)
         RPD  = PI / 180.0
         PASS1 = .FALSE.
         write(*,*) "PSNDO Check 1.9", DITHER
         call flush(6)
      END IF
      write(*,*) "PSNDO Check 1.95", DITHER
      call flush(6)
   10 CONTINUE
...
c     ..
     ...
     &          ZERO / 0.D0 / , ONE / 1.D0 /
      ...
      IER  = 0
      TOL  = epsilon(4.D0) !D1MACH( 4 ) MJA replace with F90 function
...
c     ..
     ...
     &          TWO / 2.D0 /
      ...
      IF( PI.EQ.0.0 ) THEN
         ...
         PI   = 2.*ASIN( 1.0 )
         TOL  = 10*epsilon(4.0D0) !10.*D1MACH( 4 ) MJA replace with F90 function
...
*bm   than MINRCOND
      ...
      MINRCOND = 5000. * epsilon(4.) !MJA Replace with F90 version 
...
c     .. Local Scalars ..
      ...
      LOGICAL   PASS1
c      REAL      ABSA, ABSB, HUGE, POWA, POWB, POWMAX, POWMIN, TINY
      REAL      ABSA, ABSB, POWA, POWB, POWMAX, POWMIN
c     ..
c     .. External Functions ..
...
c     ..
c      SAVE      PASS1, TINY, HUGE, POWMAX, POWMIN
...
c     ..
      ...
      IF( PASS1 ) THEN
         ...
         !TINY   = TINY(1) !R1MACH( 1 ) MJA replace with F90 functions
         !HUGE   = HUGE(2) !R1MACH( 2 )
         POWMAX = LOG10( HUGE(2.) )
         POWMIN = LOG10( TINY(1.) )
         PASS1  = .FALSE.
      ...
      ELSE
         ...
         IF( ABSA.LT.TINY(1.) .AND. ABSB.LT.TINY(1.) ) THEN
