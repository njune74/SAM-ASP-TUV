PROGRAM DMiLayTable
  ...
  INTEGER :: I, ib, ief, icore, numbin, w_ind_low
  LOGICAL :: FASTTUV
  ...
  ! absolute core radius factors - limits for bins 1-5 
  AbsCoreRadBinLim = (/0.01, 0.2, 0.4, 0.6, 0.8, 0.99/)
  ...
  DO ib = 1, numbin
     ...
     DO ief = 1, 10
        WRITE(*,*) 
        WRITE(*,*) "******************** ief = ",ief," *********************"
        ! set effective radius values (cm): average of bin limits
        InParticleEffectiveRadius =(EffRadBinLim(ief) + EffRadBinLim(ief+1))/2
        DO icore = 1, 5
           WRITE(*,*) 
           WRITE(*,*) "******************** icore = ",icore," *********************"
           ! set core radius values (cm): average of bin limits
           InParticleAbsCoreRad=(AbsCoreRadBinLim(icore)*InParticleEffectiveRadius + AbsCoreRadBinLim(icore+1)*InParticleEffectiveRadius)/2
           ...
           !WRITE(*,*) "Ending DMiLay"
           WRITE(*,*)
           WRITE(*,*) "InParticle%ExtEff(ib) = ", InParticleExtEffib
           WRITE(*,*) "InParticle%ScaEff(ib) = ", InParticleScaEffib
           WRITE(*,*) "InParticle%BackScaEff(ib) = ", InParticleBackScaEffib
           WRITE(*,*) "InParticle%AssymParam(ib) = ", InParticleAssymParamib
