...
c      PARAMETER ( MXANG = 100, LL = 1000, ZERO = 0.0D0, ONE = 1.0D0,
c     &            TWO = 2.0D0 ) !MM DEBUG - LL too small once NMX1 increased
      PARAMETER ( MXANG = 100, LL = 1050, ZERO = 0.0D0, ONE = 1.0D0,
     &            TWO = 2.0D0 )
c     ..
c     .. Scalar Arguments ..
...
c     .. External Functions ..
      ...
      LOGICAL   WRTBAD1, WRTDIM1
      EXTERNAL  WRTBAD1, WRTDIM1
c     ..
...
c     ..
      ...
      WRITE(*,*)                                !!MM DEBUG
      WRITE(*,*) "RCORE: ", RCORE              !!MM DEBUG
      WRITE(*,*) "RSHELL: ", RSHELL            !!MM DEBUG
      WRITE(*,*) "WVNO: ", WVNO            !!MM DEBUG
      WRITE(*,*) "RINDSH: ", RINDSH            !!MM DEBUG
      WRITE(*,*) "RINDCO: ", RINDCO            !!MM DEBUG
      WRITE(*,*) "MU: ", MU                  !!MM DEBUG
      WRITE(*,*) "NUMANG: ", NUMANG            !!MM DEBUG
      WRITE(*,*) "QEXT: ", QEXT            !!MM DEBUG
      WRITE(*,*) "QSCA: ", QSCA            !!MM DEBUG
      WRITE(*,*) "QBS: ", QBS            !!MM DEBUG
      WRITE(*,*) "GQSC: ", GQSC            !!MM DEBUG
      !WRITE(*,*) "M1: ", M1            !!MM DEBUG
      !WRITE(*,*) "M2: ", M2	            !!MM DEBUG		
      !WRITE(*,*) "S21: ", S21            !!MM DEBUG
      !WRITE(*,*) "D21: ", D21              !!MM DEBUG
      WRITE(*,*) "MAXANG: ", MAXANG            !!MM DEBUG
      WRITE(*,*)                               !!MM DEBUG
...
c      NMX2   = T( 1 )
      ...
      !WRITE(*,*) "NMX1 = ",NMX1 !MM DEBUG
      !WRITE(*,*) "NMX2 = ",NMX2 !MM DEBUG
c      IF( NMX1.LE.150 ) THEN

c         NMX1   = 150   
c         NMX2   = 135   !MM: not high enough, causing errors

c      END IF

c      IF( NMX1.LE.200 ) THEN
...
c                        ** Check input arguments for gross errors
      ...
      IF( WVNO.LE.0.0 ) INPERR = WRTBAD1( 'WVNO' )

c      WRITE(*,*) "RSHELL: ", RSHELL  !!MM DEBUG
      IF( RSHELL.LE.0.0 ) INPERR = WRTBAD1( 'Rshell' )

c      WRITE(*,*) "RCORE: ", RCORE  !!MM DEBUG
      IF( RCORE.LE.0.0 .OR. RCORE.GT.RSHELL ) 
     &    INPERR = WRTBAD1( 'Rcore' )

c      WRITE(*,*) "REAL(RINDSH): ", REAL(RINDSH)  !!MM DEBUG
c      WRITE(*,*) "AIMAG(RINDSH): ", AIMAG(RINDSH)  !!MM DEBUG
      IF( REAL(RINDSH).LE.0.0 .OR. AIMAG(RINDSH).GT.0.0 )
     &    INPERR = WRTBAD1( 'RindSh' )

c      WRITE(*,*) "REAL(RINDCO): ", REAL(RINDCO)  !!MM DEBUG
c      WRITE(*,*) "AIMAG(RINDCO): ", AIMAG(RINDCO)  !!MM DEBUG
      IF( REAL(RINDCO).LE.0.0 .OR. AIMAG(RINDCO).GT.0.0 ) 
     &    INPERR = WRTBAD1( 'RindCo' )

      IF( NUMANG.LT.0 ) INPERR = WRTBAD1( 'NumAng' )

      IF( NUMANG.GT.MXANG ) INPERR = WRTDIM1( 'MxAng', NUMANG )

      IF( NUMANG.GT.MAXANG ) INPERR = WRTDIM1( 'MaxAng', NUMANG )

c      WRITE(*,*) "NMX1 + 1 = ", NMX1+1
c      WRITE(*,*) "LL = ", LL
      IF( NMX1 + 1 .GT. LL ) INPERR = WRTDIM1( 'LL', NMX1 + 1 )
...
c      WRITE(*,*)                      !MM DEBUG
c      WRITE(*,*) "T(4) = ", T(4)      !MM DEBUG
      IF( T(4).GE.1.0D-14 ) THEN

         N  = N + 1

c         WRITE(*,*) "N = ",N        !MM DEBUG
c         WRITE(*,*) "NMX2 = ",NMX2  !MM DEBUG
         IF( N.GT.NMX2 ) CALL ERRMSG1(
...
c                               into elements of real scattering matrix
  ...
  140 CONTINUE
      ...
      SUBROUTINE ErrMsg1( MESSAG, FATAL )

c        Print out a warning or error message;  abort if error
c        after making symbolic dump (machine-specific)

c       Provenance:  the 3 error-handlers ErrMsg1, WrtBad, WrtDim are
...
c cccc    CALL SYMDUMP( '-B -c3' )
     ...
     &      'They will no longer be printed *******'
      ...
      LOGICAL FUNCTION WrtBad1( VARNAM )
...
c     ..
     ...
     &   '  in error  ****'
      ...
      IF( NUMMSG.EQ.MAXMSG ) CALL ERRMSG1(
     &    'Too many input errors.  Aborting...', .TRUE. )

      END

      LOGICAL FUNCTION WrtDim1( DIMNAM, MINVAL )
