...
c     .. External Subroutines ..
      ...
      EXTERNAL  BIGA, CKINMI, ERRMSG1, LPCOEF, MIPRNT, SMALL1, SMALL2,
     &          TESTMI
c     ..
...
c                              ** Number of terms in Mie series; Eq R50
      ...
      IF( NTRM+1 .GT. MAXTRM )
     &    CALL ERRMSG1('MIEV0--PARAMETER MaxTrm TOO SMALL',.TRUE.)
...
c     Routines called :  ERRMSG1, WRTBAD1, WRTDIM1
...
c     ..
      ...
      IF( XX.LT.0. ) INPERR = WRTBAD1( 'XX' )

      IF( .NOT.PERFCT .AND. REAL( CREFIN ).LE.0. )
     &    INPERR = WRTBAD1( 'CREFIN' )

      IF( MOMDIM.LT.0 ) INPERR = WRTBAD1( 'MOMDIM' )


      IF( NMOM.NE.0 ) THEN

         IF( NMOM.LT.0 .OR. NMOM.GT.MOMDIM ) INPERR = WRTBAD1( 'NMOM' )

         IF( ABS( IPOLZN ).GT.4444 ) INPERR = WRTBAD1( 'IPOLZN' )
...
c                                ** computation of cosine
   ...
   40    CONTINUE
      ...
      IF( INPERR ) CALL ERRMSG1( 'MIEV0--Input error(S).  Aborting...',
     &                          .TRUE. )

      IF( XX.GT.20000.0 .OR. REAL( CREFIN ).GT.10.0 .OR.
     &    ABS( AIMAG( CREFIN ) ).GT.10.0 )
     &    CALL ERRMSG1( 'MIEV0--XX or CREFIN outside tested range',
     &    .FALSE.)
...
c     Routines called :  ERRMSG1, LPCO1T, LPCO2T
...
c     .. External Subroutines ..
      ...
      EXTERNAL  ERRMSG1, LPCO1T, LPCO2T
c     ..
c     .. Intrinsic Functions ..
...
c     ..
   ...
   30 CONTINUE
      ...
      IF( NTRM + 2.GT.MAXTRM )
     &    CALL ERRMSG1('LPCoef--PARAMETER MaxTrm too small',.TRUE.)
...
c                                 ** Eq. R25b, k=2
   ...
   10 CONTINUE
      ...
      IF( KOUNT.GT.MAXIT )
     &    CALL ERRMSG1('ConFra--Iteration failed to converge',.TRUE.)
...
c     Routines called :  ERRMSG1, MIPRNT, TSTBAD
...
c                                    ** correct answers and abort if bad
   ...
   20    CONTINUE
     ...
     &                   SFORW, SBACK, TFORW, TBACK, S1, S2 )
            ...
            CALL ERRMSG1( 'MIEV0 -- Self-test failed',.TRUE.)
