*DECK DGAUS8
      SUBROUTINE DGAUS8 (FUN, A, B, Q, Stoich, Charge, ERR, ANS, IERR)
...
C	   Q   - is a dummy second argument passed to FUN.  (Added by HDS in 2002)
...
C              double precision unit roundoff D1MACH1(4).  ANS will
...
C
C***REFERENCES  (NONE)
C***ROUTINES CALLED  D1MACH1, I1MACH, XERMSG
...
C***END PROLOGUE  DGAUS8
     ...
     1 EPS, ERR, EST, GL, GLR, GR, HH, Q(3), Stoich(4), Charge(4), SQ2, 
     2 TOL, VL, VR, W1, W2, W3, W4, X1, X2, X3, X4, X, H
      DOUBLE PRECISION D1MACH1, G8, FUN
     ...
     2     2.22381034453374471D-01,     1.01228536290376259D-01/
      ...
      G8(X,H)=H*((W1*(FUN(X-X1*H,Q,Stoich,Charge) + 
     1				FUN(X+X1*H,Q,Stoich,Charge))
     2           +W2*(FUN(X-X2*H,Q,Stoich,Charge) + 
     3				FUN(X+X2*H,Q,Stoich,Charge)))
     4          +(W3*(FUN(X-X3*H,Q,Stoich,Charge) + 
     5				FUN(X+X3*H,Q,Stoich,Charge))
     6           +W4*(FUN(X-X4*H,Q,Stoich,Charge) + 
     7                FUN(X+X4*H,Q,Stoich,Charge))))
C***FIRST EXECUTABLE STATEMENT  DGAUS8
C
C     Initialize
C
      K = I1MACH(14)
      ANIB = D1MACH1(5)*K/0.30102000D0
      NBITS = ANIB
   ...
   10 TOL = MAX(ABS(ERR),2.0D0**(5-NBITS))/2.0D0
      IF (ERR .EQ. 0.0D0) TOL = SQRT(D1MACH1(4))
      EPS = TOL
      HH(1) = (B-A)/4.0D0
      AA(1) = A
      LR(1) = 1
...
C
 ...
 9030 FORMAT (1X)
      END
*DECK D1MACH1
      DOUBLE PRECISION FUNCTION D1MACH1 (I)
C***BEGIN PROLOGUE  D1MACH1
C***PURPOSE  Return floating point machine dependent constants.
C***LIBRARY   SLATEC
C***CATEGORY  R1
C***TYPE      DOUBLE PRECISION (R1MACH-S, D1MACH1-D)
C***KEYWORDS  MACHINE CONSTANTS
C***AUTHOR  Fox, P. A., (Bell Labs)
C           Hall, A. D., (Bell Labs)
C           Schryer, N. L., (Bell Labs)
C***DESCRIPTION
C
C   D1MACH1 can be used to obtain machine-dependent parameters for the
C   local machine environment.  It is a function subprogram with one
C   (input) argument, and can be referenced as follows:
C
C        D = D1MACH1(I)
...
C   discussed below.
C
C   D1MACH1( 1) = B**(EMIN-1), the smallest positive magnitude.
C   D1MACH1( 2) = B**EMAX*(1 - B**(-T)), the largest magnitude.
C   D1MACH1( 3) = B**(-T), the smallest relative spacing.
C   D1MACH1( 4) = B**(1-T), the largest relative spacing.
C   D1MACH1( 5) = LOG10(B)
C
...
C   To alter this function for a particular environment, the desired
C   set of DATA statements should be activated by removing the C from
C   column 1.  Also, the values of D1MACH1(1) - D1MACH1(4) should be
C   checked for consistency with the local operating system.
C
C***REFERENCES  P. A. Fox, A. D. Hall and N. L. Schryer, Framework for
C                 a portable library, ACM Transactions on Mathematical
...
C
C***FIRST EXECUTABLE STATEMENT  D1MACH1
      IF (I .LT. 1 .OR. I .GT. 5) CALL XERMSG ('SLATEC', 'D1MACH1',
     +   'I OUT OF BOUNDS', 1, 2)
C
      D1MACH1 = DMACH(I)
      RETURN
...
C   920501  Reformatted the REFERENCES section.  (WRB)
C   060630  Added flag override (MJA)
