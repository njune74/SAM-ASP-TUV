...
logical:: advect_sgs = .true. ! advect prognostics or not, default - not (Smagorinsky)
...
subroutine sgs_setparm()
  ...
  close(55)
   ...
   if(masterproc) then
      open(unit=55,file='./RUNS/'//trim(case)//'/'//trim(case)//'_'//trim(caseid)//'.nml', form='formatted', position='append')
      ...
      do j=1,ny
       do i=1,nx
         if(z(k).le.150..and..not.dosmagor) then
            !write(*,*) 'in sgs calculating tke...',tke(i,j,k),z(k)
